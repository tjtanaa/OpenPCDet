...
# Converter = PointvizConverter(home='/home/tan/tjtanaa2/OpenPCDet/visualization/southgate_preprocessing_crop')
...
def trim(data):
    ...
    keep_idx = get_union_sets([data[:, 0] > -40.0, data[:, 0] < 18.0,
                                data[:, 1] > -32.4, data[:, 1] < 41.0,
                               data[:, 2] > 0.0, data[:, 2] < 8.0])
...
class SouthGateDataset(DatasetTemplate):
    ...
    def include_south_gate_data(self, mode):
        self.logger.info('Loading South Gate dataset')
        waymo_infos = []
        ...
        self.logger.info('Total samples for South Gate dataset: %d' % (len(waymo_infos)))
    ...
    def get_infos(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1):
        ...
        print('---------------The south gate sample interval is %d, total sequecnes is %d-----------------'
              % (sampled_interval, len(self.sample_sequence_list)))


        def process_single_scene(relativefilepath):

            preprocessed_data_path = os.path.join(self.root_path, 'ProcessedData')
            print('process %s' % (relativefilepath))
            data_path = os.path.join(self.root_path, 'ProcessedData')
    ...
    def get_lidar(self, absfilepath):
        ...
        # data = south_gate_utils._retrieve_binary_data_file(bin_file, False, True)
        data = trim(data)
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            single_pred_dict['metadata'] = None #batch_dict['metadata'][index]
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        def kitti_eval(eval_det_annos, eval_gt_annos):
            from ..south_gate.south_gate_object_eval_python import eval as kitti_eval
            from ..south_gate import kitti_utils

            map_name_to_kitti = {
                "Private Cars": "Private Cars",
                "Double Decker Bus": "Double Decker Bus",
                "Single Decker Bus": "Single Decker Bus",
                "Taxi": "Taxi",
                "Mini Bus": "Mini Bus",
                "Vans": "Vans",
                ...
                "Cylindrical Truck": "Cylindrical Truck",
                "Dont Care": "Dont Care",
            }
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=1,
        ...
        for k in range(0, len(infos), sampled_interval):
            ...
            # Converter.compile(task_name="Loaded_data_" + str(k + 1),
            #                 coors=points[:,[1,2,0]],
            #                 intensity=points[:,3])
...
def create_south_gate_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    # train_filename = save_path / ('south_gate_infos_%s_test2.pkl' % train_split)
    # val_filename = save_path / ('south_gate_infos_%s_test2.pkl' % val_split)
    # train_filename = save_path / ('south_gate_infos_%s_augment.pkl' % train_split)
    # val_filename = save_path / ('south_gate_infos_%s_augment.pkl' % val_split)
...
if __name__ == '__main__':
    ...
    if args.func == 'create_south_gate_infos':
        ...
        # Open the YAML file and load its contents with the SafeLoader
        with open(args.cfg_file, 'r') as file:
            dataset_cfg = EasyDict(yaml.load(file, Loader=yaml.SafeLoader))
        # dataset_cfg = EasyDict(yaml.load(open(args.cfg_file)))
        # ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        # ROOT_DIR = Path("/media/data3/tjtanaa/south_gate_1680")
        # ROOT_DIR = Path("/media/data2/southgatetestdata2")
        ROOT_DIR = Path("/media/data3/tjtanaa/transform_south_gate_1680")
        # ROOT_DIR = Path("/media/data3/tjtanaa/transform_south_gate_full_aug_dataset")
        # ROOT_DIR = Path("/media/data2/transform_southgatetestdata2")
        # ROOT_DIR = Path("/media/data2/transform_southgatetestdata4")
    ...
    # 1680
    # Database Single Decker Bus: 190
    # Database Mini Bus: 140
    # Database Private Cars: 718
    # Database Taxi: 165
    # Database Vans: 587
    # Database Double Decker Bus: 146
    # Database Heavy Trucks: 57
    # Database Light Trucks: 47
    # Database Motorbikes: 23
    # Database Crane Truck: 67
    # Database Cylindrical Truck: 85
    # Database Medium Trucks: 55



    # test4
    # recall_roi_0.3: 0.981366
    # recall_rcnn_0.3: 0.981366
    # recall_roi_0.5: 0.950311
    # recall_rcnn_0.5: 0.962733
    # recall_roi_0.7: 0.583851
    # recall_rcnn_0.7: 0.677019
    # Average predicted number of objects(98 samples): 2.480
    # Private Cars AP@0.70, 0.70, 0.70:  
    # bbox AP:46.5045, 46.5045, 46.5045                    
    # bev  AP:24.7351, 24.7351, 24.7351                                                                                                         
    # 3d   AP:16.4093, 16.4093, 16.4093
    # aos  AP:46.28, 46.28, 46.28                               
    # Private Cars AP_R40@0.70, 0.70, 0.70:               
    # bbox AP:44.6926, 44.6926, 44.6926                                       
    # bev  AP:20.8475, 20.8475, 20.8475                                                                                                         
    # 3d   AP:11.1947, 11.1947, 11.1947                                                 
    # aos  AP:44.36, 44.36, 44.36                                                             
    # Private Cars AP@0.50, 0.50, 0.50:                        
    # bbox AP:46.5045, 46.5045, 46.5045                                                    
    # bev  AP:40.0414, 40.0414, 40.0414                                                                                                         
    # 3d   AP:35.7416, 35.7416, 35.7416                                                                                           
    # aos  AP:46.28, 46.28, 46.28               
    # Private Cars AP_R40@0.50, 0.50, 0.50:                  
    # bbox AP:44.6926, 44.6926, 44.6926                                                                                                         
    # bev  AP:36.5000, 36.5000, 36.5000 
    # 3d   AP:34.0511, 34.0511, 34.0511                                     
    # aos  AP:44.36, 44.36, 44.36                                                                                                               
    # Double Decker Bus AP@0.50, 0.50, 0.50:                                                   
    # bbox AP:71.2016, 71.2016, 71.2016                                                       
    # bev  AP:62.9630, 62.9630, 62.9630                       
    # 3d   AP:62.9630, 62.9630, 62.9630                        
    # aos  AP:70.92, 70.92, 70.92                             
    # Double Decker Bus AP_R40@0.50, 0.50, 0.50:               
    # bbox AP:68.5880, 68.5880, 68.5880                       
    # bev  AP:66.2130, 66.2130, 66.2130                        
    # 3d   AP:66.2130, 66.2130, 66.2130                                                     
    # aos  AP:68.40, 68.40, 68.40                                                                                           
    # Double Decker Bus AP@0.30, 0.25, 0.25:                                                                       
    # bbox AP:71.2016, 71.2016, 71.2016  
    # bev  AP:71.2016, 71.2016, 71.2016                                                                                                         
    # 3d   AP:71.2016, 71.2016, 71.2016
    # aos  AP:70.92, 70.92, 70.92      
    # Double Decker Bus AP_R40@0.30, 0.25, 0.25:                                          
    # bbox AP:68.5880, 68.5880, 68.5880                                                                                                         
    # bev  AP:68.5880, 68.5880, 68.5880                                                                          
    # 3d   AP:68.5880, 68.5880, 68.5880                                                
    # aos  AP:68.40, 68.40, 68.40
    # Single Decker Bus AP@0.50, 0.50, 0.50:
    # bbox AP:45.4545, 45.4545, 45.4545
    # bev  AP:45.4545, 45.4545, 45.4545
    # 3d   AP:45.4545, 45.4545, 45.4545
    # aos  AP:45.45, 45.45, 45.45
    # Single Decker Bus AP_R40@0.50, 0.50, 0.50:
    # bbox AP:47.5000, 47.5000, 47.5000
    # bev  AP:47.5000, 47.5000, 47.5000
    # 3d   AP:47.5000, 47.5000, 47.5000
    # aos  AP:47.50, 47.50, 47.50
    # Single Decker Bus AP@0.30, 0.25, 0.25:
    # bbox AP:45.4545, 45.4545, 45.4545
    # bev  AP:45.4545, 45.4545, 45.4545
    # 3d   AP:45.4545, 45.4545, 45.4545
    # aos  AP:45.45, 45.45, 45.45
    # Single Decker Bus AP_R40@0.30, 0.25, 0.25:
    # bbox AP:47.5000, 47.5000, 47.5000
    # bev  AP:47.5000, 47.5000, 47.5000
    # 3d   AP:47.5000, 47.5000, 47.5000
    # aos  AP:47.50, 47.50, 47.50
    # Vans AP@0.50, 0.50, 0.50:
    # bbox AP:17.3220, 17.3220, 17.3220
    # bev  AP:9.6803, 9.6803, 9.6803
    # 3d   AP:9.6803, 9.6803, 9.6803
    # aos  AP:12.26, 12.26, 12.26
    # Vans AP_R40@0.50, 0.50, 0.50:
    # bbox AP:15.5616, 15.5616, 15.5616
    # bev  AP:7.9954, 7.9954, 7.9954
    # 3d   AP:7.1621, 7.1621, 7.1621
    # aos  AP:11.06, 11.06, 11.06
    # Vans AP@0.30, 0.50, 0.50:
    # bbox AP:17.3220, 17.3220, 17.3220
    # bev  AP:9.6803, 9.6803, 9.6803
    # 3d   AP:9.6803, 9.6803, 9.6803
    # aos  AP:12.26, 12.26, 12.26
    # Vans AP_R40@0.30, 0.50, 0.50:
    # bbox AP:15.5616, 15.5616, 15.5616
    # bev  AP:7.9954, 7.9954, 7.9954
    # 3d   AP:7.1621, 7.1621, 7.1621
    # aos  AP:11.06, 11.06, 11.06
    ...
    # test2
    # 2021-11-05 12:52:14,036   INFO  recall_roi_0.3: 0.972789                                                                                  
    # 2021-11-05 12:52:14,036   INFO  recall_rcnn_0.3: 0.982993                                                                                 
    # 2021-11-05 12:52:14,036   INFO  recall_roi_0.5: 0.945578                                                                                  
    # 2021-11-05 12:52:14,036   INFO  recall_rcnn_0.5: 0.962585                                                                                 
    # 2021-11-05 12:52:14,036   INFO  recall_roi_0.7: 0.714286                                                                                  
    # 2021-11-05 12:52:14,036   INFO  recall_rcnn_0.7: 0.792517                                                                                 
    # 2021-11-05 12:52:14,037   INFO  Average predicted number of objects(176 samples): 3.540   
    # 2021-11-05 12:52:28,008   INFO  
    # Private Cars AP@0.70, 0.70, 0.70:  
    # bbox AP:70.5118, 70.5118, 70.5118                    
    # bev  AP:35.7143, 35.7143, 35.7143                                                                                                         
    # 3d   AP:30.5903, 30.5903, 30.5903
    # aos  AP:59.41, 59.41, 59.41                               
    # Private Cars AP_R40@0.70, 0.70, 0.70:               
    # bbox AP:70.1017, 70.1017, 70.1017                                        
    # bev  AP:36.6503, 36.6503, 36.6503                                                                                                         
    # 3d   AP:27.2941, 27.2941, 27.2941                                                 
    # aos  AP:59.10, 59.10, 59.10                                                             
    # Private Cars AP@0.50, 0.50, 0.50:                        
    # bbox AP:70.5118, 70.5118, 70.5118                                                    
    # bev  AP:50.1010, 50.1010, 50.1010                                                                                                         
    # 3d   AP:43.5078, 43.5078, 43.5078                                                                                           
    # aos  AP:59.41, 59.41, 59.41               
    # Private Cars AP_R40@0.50, 0.50, 0.50:                  
    # bbox AP:70.1017, 70.1017, 70.1017                                                                                                         
    # bev  AP:47.6756, 47.6756, 47.6756 
    # 3d   AP:43.4253, 43.4253, 43.4253                                     
    # aos  AP:59.10, 59.10, 59.10
    # Taxi AP@0.50, 0.50, 0.50:
    # bbox AP:43.8534, 43.8534, 43.8534
    # bev  AP:39.2116, 39.2116, 39.2116
    # 3d   AP:39.2116, 39.2116, 39.2116
    # aos  AP:43.67, 43.67, 43.67
    # Taxi AP_R40@0.50, 0.50, 0.50:
    # bbox AP:42.4768, 42.4768, 42.4768
    # bev  AP:37.7158, 37.7158, 37.7158
    # 3d   AP:37.7158, 37.7158, 37.7158
    # aos  AP:42.27, 42.27, 42.27
    # Taxi AP@0.30, 0.50, 0.50:
    # bbox AP:43.8534, 43.8534, 43.8534
    # bev  AP:39.2116, 39.2116, 39.2116
    # 3d   AP:39.2116, 39.2116, 39.2116
    # aos  AP:43.67, 43.67, 43.67
    # Taxi AP_R40@0.30, 0.50, 0.50:
    # bbox AP:42.4768, 42.4768, 42.4768
    # bev  AP:37.7158, 37.7158, 37.7158
    # 3d   AP:37.7158, 37.7158, 37.7158
    # aos  AP:42.27, 42.27, 42.27
    # Vans AP@0.50, 0.50, 0.50:
    # bbox AP:26.3636, 26.3636, 26.3636
    # bev  AP:26.3636, 26.3636, 26.3636
    # 3d   AP:26.3636, 26.3636, 26.3636
    # aos  AP:26.36, 26.36, 26.36
    # Vans AP_R40@0.50, 0.50, 0.50:
    # bbox AP:20.3750, 20.3750, 20.3750
    # bev  AP:20.3750, 20.3750, 20.3750
    # 3d   AP:20.3750, 20.3750, 20.3750
    # aos  AP:20.37, 20.37, 20.37
    # Vans AP@0.30, 0.50, 0.50:
    # bbox AP:26.3636, 26.3636, 26.3636
    # bev  AP:26.3636, 26.3636, 26.3636
    # 3d   AP:26.3636, 26.3636, 26.3636
    ...
    # 3d   AP:20.3750, 20.3750, 20.3750
    # aos  AP:20.37, 20.37, 20.37
    # Light Trucks AP@0.50, 0.50, 0.50:
    # bbox AP:70.1120, 70.1120, 70.1120
    # bev  AP:64.8503, 64.8503, 64.8503
    # 3d   AP:64.8503, 64.8503, 64.8503
    # aos  AP:69.10, 69.10, 69.10
    # Light Trucks AP_R40@0.50, 0.50, 0.50:
    # bbox AP:70.7247, 70.7247, 70.7247
    # bev  AP:63.9825, 63.9825, 63.9825
    # 3d   AP:63.9825, 63.9825, 63.9825
    # aos  AP:69.73, 69.73, 69.73
    # Light Trucks AP@0.30, 0.25, 0.25:
    # bbox AP:70.1120, 70.1120, 70.1120
    # bev  AP:64.8503, 64.8503, 64.8503
    # 3d   AP:64.8503, 64.8503, 64.8503
    # aos  AP:69.10, 69.10, 69.10
    # Light Trucks AP_R40@0.30, 0.25, 0.25:
    # bbox AP:70.7247, 70.7247, 70.7247
    # bev  AP:63.9825, 63.9825, 63.9825
    # 3d   AP:63.9825, 63.9825, 63.9825
    # aos  AP:69.73, 69.73, 69.73